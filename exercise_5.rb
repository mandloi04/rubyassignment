class Aircraft
    ...
    def instance_create
        for i in 0..instances.size-1
            if i==0
                instances[i] = Aircraft(1,0)
            end
            if i==1
                instances[i] = Aircraft(4,8)
            end
            if i==2
                instances[i] = Aircraft(2,10)
            end
            if i==3
                instances[i] = Aircraft(19,45)
            end
            if i==4
                instances[i] = Aircraft(32,9)
            end
        end
    end
    def final_x_y_coord
...
class Boeing_747 < Aircraft
    ...
    for i in 0..instances.size-1
        if i==0
            instances[i] = Boeing_747.new(1,1,5,8)
        end
        if i==1
            instances[i] = Boeing_747.new(12,2,9,14)
        end
        if i==2
            instances[i] = Boeing_747.new(20,15,43,6)
        end
        if i==3
            instances[i] = Boeing_747.new(9,4,64,60)
        end
        if i==4
            instances[i] = Boeing_747.new(23,19,9,50)
        end
